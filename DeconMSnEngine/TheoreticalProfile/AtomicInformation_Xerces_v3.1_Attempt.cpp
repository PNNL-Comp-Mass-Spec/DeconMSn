...
namespace Engine
{
	namespace TheoreticalProfile
	{
		...
		{			
			...
			XERCES_CPP_NAMESPACE::DOMLSSerializer *theSerializer = impl->createLSSerializer();	
			
			//theSerializer->setParameter(XMLUni::fgDOMErrorHandler,errorHandler);
			//theSerializer->setParameter(XMLUni::fgDOMWRTDiscardDefaultContent,true);
			...
			*/
			...
			if (impl != NULL)
			{
				...
				xercesc::DOMImplementation *implementation = xercesc::DOMImplementationRegistry::getDOMImplementation(L"LS"); 
				...
				if (serializer->getDomConfig()->canSetParameter(XERCES_CPP_NAMESPACE::XMLUni::fgDOMWRTFormatPrettyPrint, true)) 
					serializer->getDomConfig()->setParameter(XERCES_CPP_NAMESPACE::XMLUni::fgDOMWRTFormatPrettyPrint, true); 

				// The end-of-line sequence of characters to be used in the XML being written out.  
				serializer->setNewLine(XERCES_CPP_NAMESPACE::XMLString::transcode("\r\n"));  

				// Specify the target for the XML output. 
				XERCES_CPP_NAMESPACE::XMLFormatTarget *formatTarget = new XERCES_CPP_NAMESPACE::LocalFileFormatTarget(file_name); 

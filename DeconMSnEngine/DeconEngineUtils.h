...
namespace DeconEngine
{
	...
	public:
		...
		static double GetStandardDev(float (&intensities) __gc [], float maxIntensity) ; 
		static double GetStandardDev(std::vector<double> &intensities, float maxIntensity) ; 
		static double GetBackgroundLevel(std::vector<double> &intensities, float maxIntensity);
		static double GetBackgroundLevel(float (&intensities) __gc[], float maxIntensity);
		...
		static void SetPeaks(Engine::PeakProcessing::PeakData &pk_data, Decon2LS::Peaks::clsPeak* (&peaks) __gc []) ; 
		...
		static void Apodize(double minX, double maxX, double sampleRate, int apexPositionPercent, float (&intensities) __gc [], 
			Decon2LS::Readers::ApodizationType type) ; 
		static void UnApodize(float (&intensities) __gc[], Decon2LS::Readers::ApodizationType type) ; 

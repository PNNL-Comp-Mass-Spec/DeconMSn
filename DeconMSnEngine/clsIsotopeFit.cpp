...
namespace Decon2LS
{
	...
	double clsIsotopeFit::GetFitScore(float (&mzs) __gc [], float (&intensities) __gc [], 
		Decon2LS::Peaks::clsPeak* (&peaks) __gc [], short charge, int peak_index, double delete_intensity_threshold,
		double min_intensity_for_score, System::Collections::Hashtable* elementCounts)
	{
		...
		return mobj_fit->GetFitScore(peakData, charge, currentPeak, formula, delete_intensity_threshold, min_intensity_for_score) ; 
	}

	...
	#pragma warning(default: 4251)
		...
		public:
			...
			*/
			...
			void MovingAverageFilter(std::vector<X> &vect_mzs, std::vector<Y> &vect_intensities, int num_points){

				std::vector<Y> intensities;
				...
				intensities.push_back(vect_intensities[0]);
				intensities.push_back((vect_intensities[0]+vect_intensities[1])/2);

				for ( int i = num_points-1; i < vect_mzs.size()-num_points+1; i++){
										
					for (int j = -range; j < range+1; j++){
						sum += vect_intensities[i+j];
					}

					intensities.push_back(sum/num_points);
					sum = 0;
				}

				...
				switch (rdr->NodeType)
				{
					case XmlNodeType::Element:
						...
						else if (rdr->Name->Equals(S"OutputType"))
						{
							...
							if (rdr->Value->Equals(__box(Decon2LS::DTAGeneration::OUTPUT_TYPE::DTA)->ToString()))
							{
								this->set_OutputType(Decon2LS::DTAGeneration::OUTPUT_TYPE::DTA)  ;
							}
							else if (rdr->Value->Equals(__box(Decon2LS::DTAGeneration::OUTPUT_TYPE::LOG)->ToString()))
							{
								this->set_OutputType(Decon2LS::DTAGeneration::OUTPUT_TYPE::LOG)  ;								
							}
							else if (rdr->Value->Equals(__box(Decon2LS::DTAGeneration::OUTPUT_TYPE::CDTA)->ToString()))
							{
								this->set_OutputType(Decon2LS::DTAGeneration::OUTPUT_TYPE::CDTA)  ;								
							} 
							else if (rdr->Value->Equals(__box(Decon2LS::DTAGeneration::OUTPUT_TYPE::MGF)->ToString()))
							{
								this->set_OutputType(Decon2LS::DTAGeneration::OUTPUT_TYPE::MGF)  ;								
							} 
						}
						break;

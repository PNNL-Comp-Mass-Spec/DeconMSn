...
namespace Decon2LS
{
	clsProcRunner::clsProcRunner(void)
	{
		...
		mobj_peak_parameters = new Decon2LS::Peaks::clsPeakProcessorParameters();
		mobj_transform_parameters = new Decon2LS::HornTransform::clsHornTransformParameters();
		mobj_dta_generation_parameters = new Decon2LS::DTAGeneration::clsDTAGenerationParameters() ; 
		mobj_results = NULL ;
	}
	...
	Decon2LS::Results::clsTransformResults* clsProcRunner::CreateTransformResults(System::String *file_name,
...
//		Engine::ResultChecker::LCMSCheckResults __nogc *lcms_checker = NULL ; 
		Decon2LS::Results::clsTransformResults *transform_results = __gc new Decon2LS::Results::clsTransformResults() ;
		...
		try
		{
			...
			if (file_type == Decon2LS::Readers::FINNIGAN || file_type == Decon2LS::Readers::MZXMLRAWDATA)
				thresholded = true ;
			else
			...
			{					
				...
				else if (transform_parameters->get_SumSpectraAcrossScanRange())
				{
				...
				if (file_type == Decon2LS::Readers::PNNL_IMS)
				{
					drift_time =  ((Engine::Readers::IMSRawData *)raw_data)->GetDriftTime(scan_num) ; 
				}
				else
					drift_time = - 1; 
				...
				if (vect_mzs.size() == 0)
				{
					...
					{
						if (file_type == Decon2LS::Readers::PNNL_IMS)
							lcms_results->AddInfoForIMSScan(scan_num, 0, 0, 0, 0, 0, 0, scan_time, scan_ms_level, drift_time) ; 
						else
				...
				if (file_type != Decon2LS::Readers::PNNL_IMS && scan_num % 20 == 0)
				{
				...
				if(transform)
				{
					...
					while(found)
					{
						...
						try
						{
							...
							// AM: if summing over a window, reinsert the original intensity
								 ...
								 && transform_parameters->get_SumSpectraAcrossScanRange())
							{
								...
								{	
									...
									transformRecord.mdbl_fwhm = originalPeak.mdbl_FWHM   ; 
								}
								else
								{
	...
	void clsProcRunner::CreateDTAFile()	 
	{
		...
		try
		{
			...
			if ((strcmp(input_file_format, "RAW")==0) || (strcmp(input_file_format, "raw")==0))
				menm_file_type = Decon2LS::Readers::FileType::FINNIGAN;
			else if (strcmp(input_file_format, "mzXML")==0)
				menm_file_type = Decon2LS::Readers::FileType::MZXMLRAWDATA ; 				
			else
			{
			...
			bool thresholded ; 
			if (menm_file_type == Decon2LS::Readers::FileType::FINNIGAN || menm_file_type == Decon2LS::Readers::FileType::MZXMLRAWDATA)
				thresholded = true ;
			else
			...
			if (mobj_dta_generation_parameters->get_OutputType() == Decon2LS::DTAGeneration::OUTPUT_TYPE::LOG)		
			{
			...
			dta_processor->SetPeakParameters(mobj_peak_parameters->get_PeakBackgroundRatio(), mobj_transform_parameters->get_PeptideMinBackgroundRatio()) ; 
			...
			//Done
 			menm_state = COMPLETE ;
			clock_t current_time = clock() ; 
			int procTime = current_time - start_time ; 
			//std::cerr<<"Processing Time "<< procTime ; 
		}
		catch (char *mesg)
		{

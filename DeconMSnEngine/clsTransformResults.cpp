...
namespace Decon2LS 
{
	namespace Results
	{
		clsTransformResults::clsTransformResults(void)
		{
			...
			menmFileType = Decon2LS::Readers::FileType::UNDEFINED ; 
		}
		...
		void clsTransformResults::GetRawDataSortedInIntensity(Decon2LS::Results::clsLCMSPeak* (&lcms_peaks) __gc [])
		{
			if (mobj_lcms_results == NULL)
			{
				lcms_peaks = NULL ; 
			...
			lcms_peaks = new Decon2LS::Results::clsLCMSPeak* __gc [num_peaks] ; 
		...
		void clsTransformResults::GetRawData(Decon2LS::Results::clsLCMSPeak* (&lcms_peaks) __gc [])
		{
			if (mobj_lcms_results == NULL)
			{
				lcms_peaks = NULL ; 
		...
		void clsTransformResults::WriteScanResults(System::String *fileName)
		{
			...
			{
				DeconEngine::Utils::GetStr(fileName, fileNameCh) ; 
				if (menmFileType != Decon2LS::Readers::FileType::ICR2LSRAWDATA)
